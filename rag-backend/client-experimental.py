...
class MCPClient:
    def __init__(self):
        ...
        self.context_history_database =  chromaDB.client.get_or_create_collection(name="contextual_data", metadata={
    ...
    async def chat_loop(self):
        ...
        while True:
            try:
                ...
                if query.lower() == 'quit':
                    self.context_history_database.delete_collection(name="contextual_data")
                    break
...
async def main():
    ...
    if len(sys.argv) < 2:
        ...
        # sys.exit(1)
        server_script_path = "/Users/ayandas/Desktop/zed-proj/shield-takehome-proj/rag-chatbot-v1/rag-backend/echo.py"

    client = MCPClient()
    try:
        await client.connect_to_server(server_script_path if len(sys.argv) < 2 else sys.argv[1])
