...
@mcp.tool(
    ...
    description="seaches chroma DB to retrieve relevant context and allows control over number of relevant context user wants to retrieve (default : 5) of a particular collection. If the collection doesn't exist, new data will be created and inserted before search query is performed."
)
def retrieve_relevant_context(user_query : str = "", number_of_relevant_context : int = 5, name_of_collection : str = "complete_collection"):
...
@mcp.tool(
    ...
    description="allows for users to retrieve the topmost levels of data. (Default : 5) from the collection you want to retrieve from (default collection name : complete_collection)."
)
def get_topmost_data(number_of_rows : int
    = 5, name_of_collection : str = "complete_collection"):
    try:
...
# NOTE : this tool might cause some trouble
#
# NOTE : this tool will only work for persistent client where setting can be configured for reset logic. (but I would like to stick to HTTP client for now)
# @mcp.tool(
#     name="delete_all_collections",
#     description="This tool can be used to delete all collections and the particular data associated with it. Note that this action is irreversible and would require reloading all the data within a newly created collection"
# )
# def delete_all_data():
#     client.reset()
